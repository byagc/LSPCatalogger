...
type
  ...
  protected  //Variables internas
    ...
    OnFirstTok:
    procedure of object;
    procedure SetTokContent(tc: tFaTokContent; dStart: string; TypDelim: TFaTypeDelim; typToken: integer);
    //Manejo de bloques
  ...
  protected  //Funciones de bajo nivel
    function CreaBuscIdeEspec(out mat: TPtrATokEspec; cad: string; out i: integer; TokPos: integer = 0): boolean;
    function CreaBuscSymEspec(out mat: TPtrATokEspec; cad: string; out i: integer; TokPos: integer = 0): boolean;
    function CreaBuscEspec(out tok: TPtrTokEspec; cad: string; TokPos: integer): boolean;
    procedure TableIdent(iden: string; out mat: TPtrATokEspec; out met: TFaProcMetTable);
  ...
  public    //funciones públicas de alto nivel
    ...
    procedure DefTokContent(dStart, Content: string; typToken: integer; Complete: boolean = False);
    ...
    procedure DefTokDelim(dStart, dEnd: string; tokTyp: integer; tipDel: TFaTypeDelim = tdUniLin; havFolding: boolean = False; chrEscape: char = #0);
    ...
    function CreateBlock(blkName: string; showFold: boolean = True; parentBlk: TFaSynBlock = nil): TFaSynBlock;
    function AddBlock(dStart, dEnd: string; showFold: boolean = True; parentBlk: TFaSynBlock = nil): TFaSynBlock;
    function AddSection(dStart: string; showFold: boolean = True; parentBlk: TFaSynBlock = nil): TFaSynBlock;
    function AddFirstSection(dStart: string; showFold: boolean = True; parentBlk: TFaSynBlock = nil): TFaSynBlock;
    ...
    procedure SearchBeginEndBlock(level: integer; PosX, PosY: integer; out pIniBlock, pEndBlock: integer);
    ...
    function GetBlockInfoAtXY(XY: TPoint; out blk: TFaSynBlock; out BlockStart: TPoint; out BlockEnd: TPoint): boolean;
...
function TSynFacilSyn.CreaBuscIdeEspec(out mat: TPtrATokEspec; cad: string; out i: integer; TokPos: integer = 0): boolean;
...
function TSynFacilSyn.CreaBuscSymEspec(out mat: TPtrATokEspec; cad: string; out i: integer; TokPos: integer = 0): boolean;
...
function TSynFacilSyn.CreaBuscEspec(out tok: TPtrTokEspec; cad: string; TokPos: integer): boolean;
...
var
  ...
  i: integer;
begin
  if cad[1] in charsIniIden then
  begin  //delimitador es identificador
  ...
  begin   //el delimitador inicial es símbolo
...
procedure TSynFacilSyn.TableIdent(iden: string; out mat: TPtrATokEspec; out met: TFaProcMetTable);
...
begin
  ...
  begin  //no es sensible a la caja
    case c of
      'A', 'a': begin
        mat := @mA;
        met := @metA;
...
procedure TSynFacilSyn.SetTokContent(tc: tFaTokContent; dStart: string; TypDelim: TFaTypeDelim; typToken: integer);
...
//definición de tokens por contenido
function TSynFacilSyn.DefTokContent(dStart: string; typToken: integer): tFaTokContent;
...
procedure TSynFacilSyn.AddIdentSpec(iden: string; tokTyp: integer; TokPos: integer);
...
begin
  ...
  //Verifica si existe
  if CreaBuscIdeEspec(mat, iden, i, TokPos) then
  begin  //puede generar excepción
...
procedure TSynFacilSyn.AddIdentSpecList(listIden: string; tokTyp: integer; TokPos: integer);
...
procedure TSynFacilSyn.AddSymbSpec(symb: string; tokTyp: integer; TokPos: integer);
...
begin
  ...
  //Verifica si existe
  if CreaBuscSymEspec(mat, symb, i, TokPos) then
  begin //busca o crea
...
procedure TSynFacilSyn.AddSymbSpecList(listSym: string; tokTyp: integer; TokPos: integer);
...
procedure TSynFacilSyn.DefTokDelim(dStart, dEnd: string; tokTyp: integer; tipDel: TFaTypeDelim; havFolding: boolean; chrEscape: char);
...
var
  ...
  begin
    if r.typDel = tdNull then
    begin  //no es delimitador
      r.pRange := nil;
      exit;
    end;
    if r.dEnd = '' then exit;
    if r.dEnd = #13 then
    begin   //Como comentario de una línea
      //no puede ser multilínea
      r.pRange := @ProcEndLine;
      exit;
    end;
    //los siguientes casos pueden ser multilínea
    if r.dEnd[1] in charsIniIden then
    begin //es identificador
...
begin
  ...
  for i := 0 to maximo - 1 do
    ...
    begin
      if (length(mSym[i].txt) < length(mSym[j].txt)) then
      begin
        aux := mSym[i];
        mSym[i] := mSym[j];
        mSym[j] := aux;
  ...
  i := 0;
  while i <= High(mSym) do
  begin
    c := mSym[i].txt[1];   //toma primer caracter
    if fProcTable[c] <> @metSimbEsp then
    begin  //prepara procesamiento de delimitador
      {$IFDEF DebugMode}
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if nombre = 'TOKEN' then
        begin  //definición completa
          //lee atributos
        ...
        else if IsAttributeName(nombre) then
        begin  //definición simplificada
          //lee atributos
          tTokPos := ReadXMLParam(atri, 'TokPos');  //posición de token
    ...
    else if nombre = 'SAMPLE' then
    begin  //Cödigo de muestra
      fSampleSource := ansistring(nodo.TextContent);  //Carga texto
    end
    else if ProcXMLattribute(nodo) then
    begin
...
begin
  ...
  try
    ...
    begin
      ...
      nodo := doc.DocumentElement.ChildNodes[i];
      nombre := UpCase(ansistring(nodo.NodeName));
      if (nombre = 'IDENTIFIERS') or (nombre = 'SYMBOLS') or (nombre = 'ATTRIBUTE') or (nombre = 'COMPLETION') or (nombre = 'SAMPLE') or (nombre = '#COMMENT') then
      begin
        //No se hace nada. Solo se incluye para evitar error de "etiqueta desconocida"
        //     end else if IsAttributeName(nombre)  then begin
      end
      else if nombre = 'KEYWORD' then
      begin
        //forma corta de <TOKEN ATTRIBUTE='KEYWORD'> lista </TOKEN>
        AddIdentSpecList(ansistring(nodo.TextContent), tnKeyword);  //Carga Keywords
      end
      else if (nombre = 'TOKEN') or (nombre = 'COMMENT') or (nombre = 'STRING') then
      begin
        //Lee atributo
        if nombre = 'TOKEN' then
        begin   //Es definición formal de token
        ...
        begin   //Es definición simplificada
        ...
        if tContent.hay then
        begin //Si hay "Content", es token por contenido
        ...
        else if tEnd.hay then
        begin //definición de token delimitado
        ...
        begin  //definición incompleta
          if tStart.hay or tCharsStart.hay then
          begin //se ha indicado delimitador inicial
            ...
            begin
              ...
              if UpCAse(nodo2.NodeName) = 'REGEX' then
              begin  //instrucción
              ...
              else if UpCase(nodo2.NodeName) = '#COMMENT' then
              begin
      ...
      else if ProcXMLBlock(nodo, nil) then
      begin  //bloques válidos en cualquier parte
        //No es necesario hacer nada
      end
      else if ProcXMLSection(nodo, nil) then
      begin //secciones en "MAIN"
        //No es necesario hacer nada
      end
      else
      begin
...
begin
  ...
  if tParent.hay then
  begin //se especificó blqPad padre
  ...
  if tBackCol.hay then
  begin //lee color
    if UpCase(tBackCol.val) = 'TRANSPARENT' then blq.BackCol := COL_TRANSPAR
    else
      blq.BackCol := tBackCol.col;
  end;
  if tCloParnt.hay then
  begin
  ...
  begin
    ...
    if UpCAse(nodo2.NodeName) = 'START' then
    begin  //definición alternativa de delimitador
    ...
    else if UpCAse(nodo2.NodeName) = 'END' then
    begin  //definición alternativa de delimitador
      tTokPos := ReadXMLParam(nodo2, 'TokPos');
      CheckXMLParams(nodo2, 'TokPos');
      //agrega la referecnia del bloque al nuevo token delimitador
    ...
    else if ProcXMLSection(nodo2, blq) then
    begin  //definición de sección
      //No es necesario procesar
    end
    else if ProcXMLBlock(nodo2, blq) then
    begin  //definición de bloque anidado
      //No es necesario procesar
    end
    else if UpCase(nodo2.NodeName) = '#COMMENT' then
    begin
...
begin
  ...
  blq.IsSection := True;
  if tStart.hay then
  begin   //configuración normal con "Start"
    if tFirstSec.hay then
    begin  //hay primera sección
      AddFirstSectToTok(tStart.val, 0, blq);
    end
    else
    begin               //sección normal
      AddIniSectToTok(tStart.val, 0, blq);
    end;
  end
  else if tTokenStart.hay then
  begin  //configuración indicando nombre de token
  ...
  begin
    if UpCase(tBackCol.val) = 'TRANSPARENT' then blq.BackCol := COL_TRANSPAR
      ...
      blq.BackCol := tBackCol.col;   //lee color
  end;
...
begin
  ...
  begin
    ...
    if dSexc and (r.typDel = tdConten1) then
    begin
    ...
    else if dSexc and (r.typDel = tdConten2) then
    begin
      //Token por contenido, que se puede optimizar
      {$IFDEF DebugMode}
    ...
    else if dSexc and (r.typDel = tdConten3) then
    begin
      //Token por contenido, que se puede optimizar
      {$IFDEF DebugMode}
    ...
    else if dSexc and (r.typDel = tdConten4) then
    begin
      //Token por contenido, que se puede optimizar
      {$IFDEF DebugMode}
    ...
    else if dSexc and (r.typDel = tdUniLin) and (r.txt = r.dEnd) and (r.chrEsc = #0) then
    begin
    ...
    else if dSexc and (r.typDel = tdUniLin) and (r.dEnd = #13) then
    begin
    ...
    else if dSexc and (r.typDel = tdNull) and not r.openBlk and not r.closeBlk and not r.OpenSec then
    begin
...
begin
  ...
  while (Fold <> nil) and (Fold.BlockType <> nil) and (TFaSynBlock(Fold.BlockType).BackCol = COL_TRANSPAR) do
  begin
    Fold := Fold.Parent;
  end;
...
begin
  ...
  Success := False;
  if UpCase(blk) = 'NONE' then
  begin
    Success := True;
    exit;
  end;
  if UpCase(blk) = 'MAIN' then
  begin
    Result := MainBlk;
    Success := True;
    exit;
  ...
  for i := 0 to lisBlocks.Count - 1 do
    if Upcase(lisBlocks[i].Name) = Upcase(blk) then
    begin
 ...
 el token especial y el puntero apunte al siguiente token.}
  ...
  begin
    ...
    begin
      ...
      for blkToOpen in d.BlksToOpen do
      begin
        //verifica si se cumplen condiciones para abrir el bloque
        if blkToOpen.parentBlk = nil then
        begin //se abre en cualquier parte
        ...
        else if blkToOpen.parentBlk = CurBlk then
        begin
        ...
        else if blkToOpen.parentBlk = CurBlk_Parent then
        begin
  ...
  begin
    CurBlk := TopBlock();  //lee bloque superior (el actual)
    if curBlk.IsSection then
    begin //verifica si está en un bloque de sección
      //Estamos en un bloque de sección. ¿Será de alguna de las secciones que maneja?
      CurBlk_Parent := TopCodeFoldBlock(1);  {Lee bloque superior, en donde se ha abierto, que no
                                              siempe coincidirá con CurBlk.parentBlk }
      for SecToOpen in d.SecsToOpen do
      begin
        //verifica si se cumplen condiciones para abrir el bloque
        if SecToOpen.parentBlk = nil then
        begin //se abre en cualquier parte
        ...
        else if SecToOpen.parentBlk = CurBlk_Parent then
        begin
        ...
        else if SecToOpen.parentBlk = CurBlk_Parent.parentBlk then
        begin
        ...
        else if SecToOpen.parentBlk = CurBlk then
        begin
  ...
  begin
    ...
    if CurBlk.IsSection then
    begin //verifica si es bloque de sección
      ...
      for BlkToClose in BlksToClose do
      begin
        if BlkToClose = CurBlk_Parent then
        begin  //coincide
...
begin
  case d.typDel of
    tdNull: begin       //token que no es delimitador de token
      ...
      if d.closeBlk then
      begin //verifica primero, si es cierre de algún bloque
...
procedure TSynFacilSyn.metA_;
begin
  metIdentEsp(mA_);
end;

procedure TSynFacilSyn.metB_;
begin
  metIdentEsp(mB_);
end;

procedure TSynFacilSyn.metC_;
begin
  metIdentEsp(mC_);
end;

procedure TSynFacilSyn.metD_;
begin
  metIdentEsp(mD_);
end;

procedure TSynFacilSyn.metE_;
begin
  metIdentEsp(mE_);
end;

procedure TSynFacilSyn.metF_;
begin
  metIdentEsp(mF_);
end;

procedure TSynFacilSyn.metG_;
begin
  metIdentEsp(mG_);
end;

procedure TSynFacilSyn.metH_;
begin
  metIdentEsp(mH_);
end;

procedure TSynFacilSyn.metI_;
begin
  metIdentEsp(mI_);
end;

procedure TSynFacilSyn.metJ_;
begin
  metIdentEsp(mJ_);
end;

procedure TSynFacilSyn.metK_;
begin
  metIdentEsp(mK_);
end;

procedure TSynFacilSyn.metL_;
begin
  metIdentEsp(mL_);
end;

procedure TSynFacilSyn.metM_;
begin
  metIdentEsp(mM_);
end;

procedure TSynFacilSyn.metN_;
begin
  metIdentEsp(mN_);
end;

procedure TSynFacilSyn.metO_;
begin
  metIdentEsp(mO_);
end;

procedure TSynFacilSyn.metP_;
begin
  metIdentEsp(mP_);
end;

procedure TSynFacilSyn.metQ_;
begin
  metIdentEsp(mQ_);
end;

procedure TSynFacilSyn.metR_;
begin
  metIdentEsp(mR_);
end;

procedure TSynFacilSyn.metS_;
begin
  metIdentEsp(mS_);
end;

procedure TSynFacilSyn.metT_;
begin
  metIdentEsp(mT_);
end;

procedure TSynFacilSyn.metU_;
begin
  metIdentEsp(mU_);
end;

procedure TSynFacilSyn.metV_;
begin
  metIdentEsp(mV_);
end;

procedure TSynFacilSyn.metW_;
begin
  metIdentEsp(mW_);
end;

procedure TSynFacilSyn.metX_;
begin
  metIdentEsp(mX_);
end;

procedure TSynFacilSyn.metY_;
begin
  metIdentEsp(mY_);
end;

procedure TSynFacilSyn.metZ_;
begin
  metIdentEsp(mZ_);
end;
...
begin
  ...
  for i := 0 to High(mSym) do
  begin  //se empieza con los de mayor tamaño
...
begin
  ...
  begin
    if fLine[posFin] = charIni then
    begin //busca fin de cadena
      Inc(posFin);
...
begin
  //busca delimitador final
  if chrEsc = #0 then
  begin  //no hay caracter de escape
    p := strscan(fLine + posFin, delTok[1]);
  end
  else
  begin  //debe filtrar el caracter de escape
    p := strscan(fLine + posFin, delTok[1]);
    while (p <> nil) and ((p - 1)^ = chrEsc) do
    begin
...
begin
  ...
  while p <> nil do
  begin   //definitivamente no se encuentra
    ...
    if (c1 in charsIniIden) or CharsIdentif[c1] or CharsIdentif[c2] then
    begin
    ...
    begin  //es el identificador buscado
...
function TSynFacilSyn.CreateBlock(blkName: string; showFold: boolean = True; parentBlk: TFaSynBlock = nil): TFaSynBlock;
...
function TSynFacilSyn.AddBlock(dStart, dEnd: string; showFold: boolean = True; parentBlk: TFaSynBlock = nil): TFaSynBlock;
...
function TSynFacilSyn.AddSection(dStart: string; showFold: boolean = True; parentBlk: TFaSynBlock = nil): TFaSynBlock;
...
function TSynFacilSyn.AddFirstSection(dStart: string; showFold: boolean = True; parentBlk: TFaSynBlock = nil): TFaSynBlock;
...
{  Line := CurrentLines[PosY];
  ...
  begin
    Start := GetTokenPos + 1;
    if (PosX >= Start) and (PosX < posFin + 1) then
    begin
      //encontró
...
function TSynFacilSyn.ExploreLine(XY: TPoint; out toks: TATokInfo; out CurTok: integer): boolean;
...
begin
  ...
  begin
    ...
    if (PosX > PosIni) and (PosX < posFin + 1) then
    begin
      //encontró
...
begin
  ...
  begin
    ...
    if (niv1 < level) and (niv2 >= level) then
    begin
...
procedure TSynFacilSyn.SearchBeginEndBlock(level: integer; PosX, PosY: integer; out pIniBlock, pEndBlock: integer);
...
begin
  ...
  begin
    ...
    begin        //aún no pasa al token actual
      if (niv1 < level) and (niv2 >= level) then
      begin
    ...
    //verifica
    if (PosX >= posIni + 1) and (PosX < posFin + 1) then
    begin
  ...
  if Despues then
  begin  //ya pasó al token actual, pero no encontró el cierre
    //hace la verificación del último token
    niv2 := NestedBlocks;   //lee nivel después del último Next()
    if (niv1 >= level) and (niv2 < level) then
    begin
...
procedure TSynFacilSyn.GetBlockInfoAtXY(XY: TPoint; out blk: TFaSynBlock; out level: integer);
...
function TSynFacilSyn.GetBlockInfoAtXY(XY: TPoint; out blk: TFaSynBlock; out BlockStart: TPoint; out BlockEnd: TPoint): boolean;
...
begin
  ...
  if BlockStart.x = -1 then
  begin  //no se encontró en la línea actual
    while (BlockStart.y > 1) do
    begin
  ...
  if BlockEnd.x = MAXINT then
  begin  //no se encontró en la línea actual
    while (BlockEnd.y < CurrentLines.Count) do
    begin
...
begin
  ...
  if CloseThisBlk <> nil then
  begin
    EndBlockFa(CloseThisBlk);
    if CloseThisBlk.CloseParent then
    begin
  ...
  begin
    if posFin = tamLin then
    begin  //para acelerar la exploración
      fTokenID := tnEol;
      exit;
    end;
